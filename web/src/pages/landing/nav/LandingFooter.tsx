...
export function LandingFooter() {
  ...
  return (
    <footer id={SECTION_IDS.contact} className="relative bg-paper py-16 text-ink">
      ...
      <div className="mx-auto max-w-5xl px-5">
        <div className="grid gap-12 md:grid-cols-2">
          <motion.div initial="hidden" whileInView="show" viewport={VP} variants={stagger()}>
            ...
            <motion.p variants={fadeUp} className="max-w-xs text-sm leading-relaxed text-ink-soft">
              {d.footer.tagline}
            </motion.p>
            <motion.div variants={fadeUp} className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm text-ink-soft">
          ...
          <motion.div initial="hidden" whileInView="show" viewport={VP} variants={fadeUp}>
            <h3 className="mb-5 font-serif text-lg italic text-ink-soft">{d.footer.contactForm.heading}</h3>
        ...
        <div className="mt-14 flex flex-col items-center justify-between gap-3 border-t border-ink/10 pt-6 md:flex-row">
          <p className="text-xs text-ink-soft">© {new Date().getFullYear()} kolab. {d.footer.rights}</p>
          <p className="text-xs text-ink-soft">{d.footer.madeIn} 🇳🇵</p>
        </div>
      </div>
    </footer>
  );
