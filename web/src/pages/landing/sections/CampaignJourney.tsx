  ...
  return (
    <section id={SECTION_IDS.journey} className="relative overflow-hidden bg-white py-28">
      ...
      <div className="relative mx-auto max-w-5xl px-6">
        <div className="grid items-start gap-14 lg:grid-cols-[1fr_1.15fr] lg:gap-12">
          {/* ── Copy column ── */}
          <motion.div initial="hidden" whileInView="show" viewport={VP} variants={stagger()} className="lg:sticky lg:top-32">
            ...
            <motion.p variants={fadeUp} className="mt-4 max-w-md text-ink-soft">
              {d.journey.sub}
            </motion.p>
          ...
          <motion.div ref={cardRef} initial="hidden" whileInView="show" viewport={VP} variants={fadeUp}>
            <div className="rounded-3xl border border-ink/10 bg-white p-6 shadow-[0_30px_60px_-24px_rgba(20,17,16,0.18)] sm:p-7">
                ...
                {steps.map((step, i) => {
                  ...
                  return (
                    <li key={i} className="relative flex gap-4 pb-7 last:pb-0">
                      {i < steps.length - 1 && (
                        <span aria-hidden className="absolute left-[17px] top-9 h-[calc(100%-2.25rem)] w-[2px] bg-ink/10">
                          <motion.span
                            className="block w-full origin-top bg-emerald-500"
                            initial={false}
                      ...
                      >
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className={`text-sm font-bold ${status === 'pending' ? 'text-ink/50' : 'text-ink'}`}>
                            {step.title}
                          </h3>
                          <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${ROLE_STYLE[role]}`}>
