...
const ROLE_STYLE: Record<Role, string> = {
  ...
  system: 'bg-ink/8 text-ink-soft dark:bg-white/10 dark:text-white/60',
};
const BADGE_STYLE: Record<Role, string> = {
  ...
  // Opaque dark chip in light mode (a translucent one would wash out against
  // the light page bg, leaving the white icon on top unreadable) — reverts to
  // the original frosted-glass-on-ink treatment in dark mode.
  system: 'bg-ink/80 shadow-[0_6px_16px_-4px_rgba(20,17,16,0.35)] dark:bg-white/15 dark:shadow-[0_6px_16px_-4px_rgba(255,255,255,0.15)]',
};
...
export function CampaignJourney() {
  ...
  return (
    <section id="journey" className="relative overflow-hidden bg-paper py-28 text-ink dark:bg-ink dark:text-white">
      ...
      <div ref={sectionRef} className="relative mx-auto max-w-6xl px-6">
        ...
        >
          ...
          <div className="relative mb-12 h-2.5">
            <div className="absolute left-0 right-0 top-1/2 h-px -translate-y-1/2 bg-ink/10 dark:bg-white/10" />
            <motion.div
                ...
                <button
                  ...
                  className={`h-2.5 w-2.5 rounded-full border-2 transition-all duration-500 ${
                    i === active
                      ...
                      : i < active
                        ? 'border-transparent bg-ink/40 dark:bg-white/40' // already passed
                        : 'border-ink/20 bg-paper hover:border-ink/40 dark:border-white/20 dark:bg-ink dark:hover:border-white/40' // upcoming
                  }`}
                />
              ))}
            </div>
              ...
              >
                <div>
                  ...
                  <div className="mt-5 flex items-center gap-4">
                    ...
                    <span className="font-mono text-xs tracking-[0.3em] text-ink/35 dark:text-white/35">
                      {String(active + 1).padStart(2, '0')} / {String(steps.length).padStart(2, '0')}
                    </span>
                  </div>
                  <h3 className="mt-5 text-balance font-serif text-3xl font-medium leading-tight tracking-tight text-ink md:text-4xl dark:text-white">
                    {step.title}
                  </h3>
                  <p className="mt-4 max-w-md text-base leading-relaxed text-ink-soft dark:text-white/60">{step.desc}</p>
                </div>
