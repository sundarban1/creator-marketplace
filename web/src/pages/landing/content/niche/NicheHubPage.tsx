...
export function NicheHubPage({ slug, seoTitle, seoDescription, keywords, breadcrumbName, icon, eyebrow, heading, intro, gridHeading, items, ctaHeading, ctaSub }: NicheHubPageProps) {
  const path = `/${slug}`;

  return (
    <ContentPageLayout
      seo={{ title: seoTitle, description: seoDescription, path, keywords, jsonLd: webPageSchema({ path, title: seoTitle, description: seoDescription }) }}
      breadcrumb={[{ name: 'Home', path: '/' }, { name: breadcrumbName, path }]}
      icon={icon}
      eyebrow={eyebrow}
      heading={heading}
      intro={intro}
      related={CORE_RELATED}
      cta={{ heading: ctaHeading, sub: ctaSub }}
    >
      <ContentSection heading={gridHeading}>
        <div className="grid gap-4 sm:grid-cols-2">
          {items.map((item) => (
            ...
            >
              ...
              <p className="mt-1.5 text-sm leading-relaxed text-ink-soft">{item.eyebrow}</p>
            </Link>
          ))}
        </div>
      </ContentSection>
