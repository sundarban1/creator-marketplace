...
import type { Platform, PlatformStatus } from '../../context/PlatformsContext';
...
interface PlatformFormProps {
  initial?: Platform;
  onSubmit: (data: Omit<Platform, 'id' | 'createdAt' | 'campaignCount'>) => void;
  submitLabel: string;
}

export function PlatformForm({ initial, onSubmit, submitLabel }: PlatformFormProps) {
  ...
  const [status, setStatus] = useState<PlatformStatus>(initial?.status ?? 'active');
  ...
  useEffect(() => {
    if (!keyTouched) setKey(slugify(name));
  }, [name, keyTouched]);
  ...
  function handleSubmit(e: FormEvent) {
    ...
    onSubmit({ icon: icon.trim(), iconBg, color, name: name.trim(), key: key.trim(), status });
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Search icons, e.g. instagram, tiktok, youtube…"
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
            <h3 className="text-sm font-semibold text-gray-800">Platform Details</h3>
            <div>
              <label htmlFor="plt-name" className="block text-xs font-medium text-gray-600 mb-1.5">
            ...
            <div>
              <label htmlFor="plt-key" className="block text-xs font-medium text-gray-600 mb-1.5">
              ...
              {!errors.key && (
                <p className="text-xs text-gray-400 mt-1">Full key: <span className="font-mono">platform/{key || '…'}</span></p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="flex flex-col gap-2">
              {(['active', 'inactive'] as PlatformStatus[]).map((s) => (
                <button
                  key={s}
                  type="button"
                  onClick={() => setStatus(s)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border-2 transition-all ${
                    status === s
                      ? s === 'active' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-400 bg-gray-50'
