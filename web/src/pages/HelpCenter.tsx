...
function ArticleModal({
  ...
  initial: Partial<HelpArticle> | null;
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-bold text-gray-900">{initial?.id ? 'Edit Article' : 'New Help Article'}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              value={form.answer}
              onChange={(e) => { setForm((f) => ({ ...f, answer: e.target.value })); setErrors((err) => ({ ...err, answer: '' })); }}
...
export function HelpCenter() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await api.help.listAll();
  ...
  async function handleSave(form: typeof EMPTY_FORM) {
    ...
    try {
      if (editTarget?.id) {
        await api.help.update(editTarget.id, form);
        showToast('Article updated');
      } else {
        await api.help.create(form);
      ...
      showToast((err as Error).message ?? 'Something went wrong');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function togglePublish(article: HelpArticle) {
    ...
    try {
      await api.help.togglePublish(article.id, next);
      showToast(next ? 'Article published' : 'Article unpublished');
    } catch {
      setArticles((prev) => prev.map((a) => a.id === article.id ? { ...a, published: article.published } : a));
        ...
        <div className="flex gap-2 flex-wrap">
          {categories.map((c) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors ${filterCat === c ? 'bg-indigo-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-indigo-400 hover:text-indigo-600'}`}
            >
              {c}
            </button>
          ))}
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm font-medium text-gray-700">{articles.length === 0 ? 'No articles yet' : 'No matches found'}</p>
            <p className="text-xs text-gray-400 mt-1">{articles.length === 0 ? 'Click "New Article" to create your first help article.' : 'Try a different search or category.'}</p>
          </div>
        ) : (
              ...
              {filtered.map((article) => (
                <tr key={article.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <p className="font-medium text-gray-800 line-clamp-1">{article.question}</p>
                    <p className="text-xs text-gray-400 mt-0.5 line-clamp-1">{article.answer}</p>
                  </td>
                  ...
                  <td className="px-4 py-3 hidden lg:table-cell text-gray-500 text-xs font-mono">#{article.order}</td>
                  <td className="px-4 py-3">
                    <button
                    ...
                    >
                      {article.published ? <><Eye size={11} /> Published</> : <><EyeOff size={11} /> Draft</>}
                    </button>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() => { setEditTarget(article); setShowModal(true); }}
