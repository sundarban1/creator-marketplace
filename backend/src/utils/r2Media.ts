...
export type R2Category = 'videos' | 'audio';
...
export type R2SinglePlan = { provider: 'r2'; mode: 'single'; key: string; uploadUrl: string; expiresIn: number };
...
async function r2VideoPlan(ownerId: string, ext: string, contentType: string, sizeBytes: number): Promise<R2SinglePlan | R2MultipartPlan> {
  ...
  if (sizeBytes < MULTIPART_THRESHOLD_BYTES) {
    ...
    return { provider: 'r2', mode: 'single', key, uploadUrl, expiresIn };
  }
  ...
  return { provider: 'r2', mode: 'multipart', key, uploadId, partSize: MULTIPART_PART_SIZE_BYTES, parts, expiresIn };
}
